...
const Navigation = () => {
  ...
  return (
    <nav className="glass-morphism h-20 flex items-center justify-around px-4 relative z-50 safe-area-bottom shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
      ...
      <NavItem to="/marketplace" icon={ShoppingCart} label="Pharma" active={location.pathname === '/marketplace'} />
      <NavItem to="/clinics" icon={Building2} label="Clinics" active={location.pathname === '/clinics'} />
      <NavItem to="/tracking" icon={Truck} label="Track" active={location.pathname === '/tracking'} />
      <NavItem to="/dashboards" icon={LayoutDashboard} label="Admin" active={location.pathname.startsWith('/dashboards')} />
    </nav>
  );
};
