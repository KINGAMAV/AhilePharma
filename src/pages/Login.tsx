...
import { Mail, Lock, Phone, User, Stethoscope, Pill, Building2, Truck, ChevronLeft } from 'lucide-react';
...
export default function Login() {
  ...
  const handleAuth = (e: React.FormEvent) => {
    ...
    if (selectedRole === 'Patient') {
      ...
      navigate(`/dashboards/${selectedRole.toLowerCase()}`);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col p-8 overflow-y-auto">
      ...
      <form onSubmit={handleAuth} className="space-y-4">
        ...
        {isLogin && (
          <div className="flex justify-end">
            <button type="button" className="text-sm font-medium text-primary">Mot de passe oublié ?</button>
          </div>
        )}
        ...
        <motion.button
          whileTap={{ scale: 0.98 }}
          type="submit"
          className="w-full h-14 rounded-xl bg-primary text-white font-bold text-lg shadow-lg shadow-primary/20 mt-4"
        >
      ...
      <div className="mt-auto pt-10 flex flex-col items-center space-y-4">
        <span className="text-xs text-slate-400 uppercase tracking-widest font-bold">Ou continuer avec</span>
